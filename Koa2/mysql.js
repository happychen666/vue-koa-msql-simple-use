...
const createPosts = (val) => {
  // 新建posts
  let _sql = `INSERT INTO posts (userName, userId, avator, title, content, hot, comments, createTime) VALUES (?,?,?,?,?,?,?,?)`;
  return query(_sql, val);
};

const updatePosts = (val) => {
  // 修改posts
  let _sql = `UPDATE posts SET title=?, content=? WHERE id=?`;
  return query(_sql, val);
};

const updatePostsComment = (val) => {
  // 修改posts评论数量
  let _sql = `UPDATE posts SET comments=? WHERE id=?`;
  return query(_sql, val);
};

const updatePostsHot = (val) => {
  // 修改posts查看人数
  let _sql = `UPDATE posts SET hot=? WHERE id=?`;
  return query(_sql, val);
};

const postsList = (key, pg, size) => {
  // 查找所有posts
  console.log("操作数据库 postsList");
  let _sql = `SELECT * FROM posts ${
    key ? "WHERE title LIKE '%" + key + "%' " : " "
  }ORDER BY createTime DESC limit ${pg * size} , ${size}`;
  return query(_sql);
};

const postDetail = (val) => {
  // 根据ID 查询 postsDetail
